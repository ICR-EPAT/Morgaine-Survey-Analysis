...
estimand_model$estimand_num <- factor(ifelse(estimand_model$estimand_num == 1, "Yes", "No"),
                                      levels = c("No", "Yes"))
...
ggplot(estimand_model, aes(x = sponsor, y = estimand_num)) +
  geom_jitter(height = 0.05,
              alpha = 0.1) +
  labs(title = "Distribution of Estimand usage by Sponsor",
       x = "Sponsor",
       y = "Estimand used")
...
ggplot(estimand_model_recode1, aes(x = sponsor, y = estimand_num)) +
  geom_jitter(height = 0.05,
              alpha = 0.1) +
  labs(title = "Distribution of Estimand usage by Sponsor",
       x = "Sponsor",
...
ggplot(estimand_model_recode1, aes(x = funding, y = estimand_num)) +
  geom_jitter(height = 0.05,
              alpha = 0.1) +
  labs(title = "Distribution of Estimand usage by Funding",
       x = "Funding",
       y = "Estimand used")
...
ggplot(estimand_model_recode1, aes(x = cancer, y = estimand_num)) +
  geom_jitter(height = 0.05,
              alpha = 0.1) +
  labs(title = "Distribution of Estimand usage by Cancer vs Non-Cancer",
       x = "Cancer vs Non-Cancer",
       y = "Estimand used")
...
ggplot(estimand_model_recode1, aes(x = year, y = estimand_num)) +
  geom_jitter(height = 0.05,
              alpha = 0.1) +
  labs(title = "Distribution of Estimand usage by Year",
       x = "Year",
       y = "Estimand used")
...
ggplot(estimand_model_recode1, aes(x = uk, y = estimand_num)) +
  geom_jitter(height = 0.05,
              alpha = 0.1) +
  labs(title = "Distribution of Estimand usage by UK Trial Status",
       x = "UK Trial Status",
       y = "Estimand used")
...
ggplot(estimand_model_recode1, aes(x = country, y = estimand_num)) +
  geom_jitter(height = 0.05,
              alpha = 0.1) +
  labs(title = "Distribution of Estimand usage by Country",
       x = "Country",
       y = "Estimand used")
...
ggplot(estimand_model_recode1, aes(x = phase, y = estimand_num)) +
  geom_jitter(height = 0.05,
              alpha = 0.1) +
  labs(title = "Distribution of Estimand usage by Phase",
       x = "Phase",
       y = "Estimand used")
...
ggplot(estimand_model_recode1, aes(x = random, y = estimand_num)) +
  geom_jitter(height = 0.05,
              alpha = 0.1) +
  labs(title = "Distribution of Estimand usage by Randomisation Allocation",
       x = "Randomisation Allocation",
       y = "Estimand used")
...
ggplot(estimand_model_recode1, aes(x = int_model, y = estimand_num)) +
  geom_jitter(height = 0.05,
              alpha = 0.1) +
  labs(title = "Distribution of Estimand usage by Intervention Model",
...
model2 <- glm(estimand_num ~ sponsor + funding + cancer + year + uk + country + random + phase, data = estimand_model_recode1, family = binomial(link = "logit"))
...
#sponsor    2.376105  2        1.241556
#funding    1.559004  3        1.076815
#cancer     1.103573  1        1.050511
#year       1.312488  3        1.046363
#uk         1.336801  1        1.156201
#country    1.363785  1        1.167812
#random     8.346138  2        1.699697
#int_model 18.761258  5        1.340683
#phase      1.449589  5        1.037826

#Check model using GAMLSS
par(mfrow=c(1,1))
library(gamlss)
model2.1 <- gamlss(estimand_num ~ sponsor + funding + cancer + year + uk + country + random + phase, family = BI, data = estimand_model_recode1)
plot(model2.1)
...
pred_df <- data.frame(year = estimand_model_recode1$year, sponsor = estimand_model_recode1$sponsor, cancer = estimand_model_recode1$cancer, prob = pred_probs)
pred_df$year <- factor(pred_df$year)
pred_df$sponsor <- factor(pred_df$sponsor)
pred_df$cancer <- factor(pred_df$cancer)
...
filtered_sponsor3 <- pred_df %>%
  filter(cancer %in% c("Non-cancer", "Cancer"))

ggplot(filtered_sponsor3, aes(x = year, y = prob)) +
  geom_point(aes(color = year), size = 1.2) +  # Scatter plot with points colored by year
  geom_smooth(aes(color = cancer, group = cancer), method = "loess", linetype = "dotted", size = 0.8, span = 0.75, se = TRUE) +  # Loess line for each industry type
  labs(x = "Year", y = "Predicted Probability", color = "Cancer vs Non-cancer") +  # Add axis labels and legend title
  scale_y_continuous(limits = c(0, 1)) +  # Set y-axis limits between 0 and 1
  theme_minimal() +
  ggtitle("Predicted Probabilities for 2021-2024 by Cancer vs Non-Cancer (with LOESS)")

```

#Collinearity check

```{r collinearity matrix, include=TRUE}

#Data used for logistic regression

library(ggcorrplot)
model.matrix(~0+., data=estimand_model_recode1) %>% 
  cor(use="pairwise.complete.obs") %>% 
  ggcorrplot(show.diag=FALSE, type="lower", lab=TRUE, lab_size=2)

#Recoded

estimand_model_recode2 <- estimand_model_recode1

estimand_model_recode2$sponsor <- factor(estimand_model_recode2$sponsor, levels = c("Industry", "Non-Industry", "Missing"))
estimand_model_recode2$funding <- factor(estimand_model_recode2$funding, levels = c("Industry", "Non-Industry", "Other", "Missing"))
estimand_model_recode2$cancer <- factor(estimand_model_recode2$cancer, levels = c("Cancer", "Non-cancer"))
estimand_model_recode2$uk <- factor(estimand_model_recode2$uk, levels = c("Yes", "No"))
estimand_model_recode2$country <- factor(estimand_model_recode2$country, levels = c("Multiple", "Single"))

library(ggcorrplot)
model.matrix(~0+., data=estimand_model_recode2) %>% 
  cor(use="pairwise.complete.obs") %>% 
  ggcorrplot(show.diag=FALSE, type="lower", lab=TRUE, lab_size=2)
