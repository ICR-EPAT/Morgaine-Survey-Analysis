...
q8_bar <- mssurvey_cc$Q8 %>% as.data.frame %>%
  ...
  geom_text(aes(label=N),vjust=-0.25,fontface='bold')+
    labs(
    title = "Q8: Thinking about the last 3 years, how often has the estimand framework been used when new trials are being developed?",
    x = "Question 8",
    y = "Count"
  )
  theme_bw()+
...
q19_bar <- q19.1$publication2 %>% as.data.frame %>%
  ...
  geom_text(aes(label=N),vjust=-0.25,fontface='bold')+
    labs(
    title = "Q19: If no publications to date have reported on estimand(s) used, in which trial publications are you planning to report details of the estimand(s) used?",
    x = "Question 19",
    y = "Count"
  )
  theme_bw()+
